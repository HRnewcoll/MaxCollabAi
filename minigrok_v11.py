...
# ║                                                                  ║
# ║  NEW IN v11 (addresses all major AI review feedback):           ║
...
_pip("gradio>=4.20.0", "matplotlib")
_pip("sympy", silent=False)
_pip("pytest", silent=False)     # v11: test framework
...
import json, shutil, threading, tempfile, re, traceback, warnings, logging
...
import cv2, whisper as whisper_lib
...
class Config:
    """All tunable constants in one place — edit here, nowhere else."""

    # ── Model ────────────────────────────────────────────────────────
    MODEL_ID        = "Qwen/Qwen2.5-VL-7B-Instruct"          # v11: fixed model ID
    MODEL_FALLBACKS = ["Qwen/Qwen2-VL-7B-Instruct"]
    MIN_VRAM_GB     = 5.5

    # ── Feature Toggles (set False to save VRAM / skip slow installs) ─
    ENABLE_IMAGE_GEN   = True   # Stable Diffusion (uses ~4GB extra VRAM when active)
    ENABLE_VOICE       = True   # TTS + Whisper STT
    ENABLE_BROWSER     = True   # Playwright browser agent
    ENABLE_BG_LEARNING = True   # Background arXiv/RSS learner
    ENABLE_REFLECTION  = True   # Self-critique loop after complex answers
    USE_LARGE_EMBEDDER = False  # True=BAAI/bge-m3 (better), False=MiniLM (faster, less VRAM)
    ...
    TOOL_CACHE_SIZE  = 100
    TOOL_CACHE_TTL   = 300
    CACHE_TTLS       = {"factual":3600,"news":300,"code":1800,"creative":0,"personal":0,"math":7200}
    ...
    # ── Training ─────────────────────────────────────────────────────
    KERNEL_VAR_LIMIT  = 500
    RATE_LIMIT_RPM    = 30
    BG_LEARN_INTERVAL = 7200
    LORA_R            = 16
    LORA_ALPHA        = 32
    LORA_DROPOUT      = 0.05
    LORA_TARGETS      = ["q_proj","k_proj","v_proj","o_proj","gate_proj","up_proj","down_proj"]
    # DPO: corrections are queued to disk, never run inline (prevents UI freeze)
    DPO_BATCH_MIN     = 10
...
MODEL_SEARCH = [f"{DRIVE_ROOT}/minigrok_v{v}/model" for v in ["10", "9", "8", "7"]] + \
               [f"{DRIVE_ROOT}/mini_grok_model"]
DR        = f"{DRIVE_ROOT}/minigrok_v10"
...
def audit(event, data=None):
    ...
    with open(AUDIT_LOG, "a") as f:
        f.write(json.dumps({"ts": datetime.now().isoformat(), "event": event, **(data or {})}) + "\n")
...
def _load_model(model_dir):
    """Load the VL model with best available attention implementation."""
    free = _vram_free()
    ...
    proc = AutoProcessor.from_pretrained(
        model_dir, trust_remote_code=True, padding_side="left", extra_special_tokens={}
    )
    ...
    for attn in ["flash_attention_2", "sdpa", "eager"]:
        try:
            ...
            log.info(f"  Attention: {attn}")
            return proc, mdl
        except torch.cuda.OutOfMemoryError:
            torch.cuda.empty_cache()
        except Exception:
            pass
    raise RuntimeError("Failed to load model with any attention backend.")
...
log.info(f"Model loaded | VRAM: {torch.cuda.memory_allocated()/1e9:.1f}GB | Context: {MODEL_MAX_TOKENS:,} tokens")
...
def _get_whisper(size="base"):
    if size not in _whisper_models:
        _whisper_models[size] = whisper_lib.load_model(size)
...
def record_feedback(question, chosen, rejected):
    """Record a DPO training pair from user feedback or auto-correction."""
    _dpo_pairs.append({
        "prompt": question, "chosen": chosen, "rejected": rejected,
        "ts": datetime.now().isoformat()
    })
    DPO_PATH.write_text(json.dumps(_dpo_pairs, indent=2))
    if len(_dpo_pairs) % 20 == 0:
        _improve_q.put("dpo")
...
def auto_correct(question, bad_answer, reason="low_confidence") -> dict:
    ...
    CORR_PATH.write_text(json.dumps(_correction_log[-200:], indent=2))
    return result
...
def _t_deep_research(topic, depth=3) -> str:
    ...
    try:
        raw = quick(f"List {depth} subtopics for '{topic}' as JSON array.", max_tokens=60, temp=0.4)
        m = re.search(r'\[.*?\]', raw, re.DOTALL)
...
def solve_homework(question, subject="auto", show_working=True) -> str:
    ...
    if subject == "maths":
        try:
            code = quick(f"Write SymPy code to solve: {question}\nReturn ONLY code.", max_tokens=200, temp=0.1)
    ...
    solution = quick(prompt, system=sys_p, max_tokens=1200, temp=0.3)
...
def _load_plugins():
    ...
    for plugin_file in plugin_dir.glob("*.py"):
        try:
            ...
            log.warning(f"  Plugin {plugin_file.stem} failed: {e}")
    return loaded
    ...
    for topic in _learn_topics:
        try:
            for paper in arxiv_lib.Client().results(
                ...
                try:
                    raw = quick(f"Generate Q&A about this paper:\n{paper.title}\n{paper.summary[:400]}\n"
...
def run_eval(label="eval", log_fn=None) -> dict:
    ...
    for t, v in by_type.items():
        _log(f"  {t:12s}: {v:.1%}")
    return {"label": label, "overall": round(overall, 3), "by_type": by_type, "timestamp": datetime.now().isoformat()}
...
log.info("RAG audit + system status ready")
...
# v11: Agent state tracker — observable from Status tab and logs
_agent_state: dict = {
    "last_message": "",
    "last_ts": "",
...
def agent_stream(message, history, image_path=None, log_fn=None):
    ...
    for tool_round in range(5):
        ...
        if tool_name not in TOOL_REGISTRY:
            ...
            break
        if log_fn:
            log_fn(f"  🔧 {tool_name}({', '.join(f'{k}={str(v)[:30]}' for k, v in params.items())})")
        try:
...
def create_ui():
    with gr.Blocks(theme=gr.themes.Soft(primary_hue="blue", neutral_hue="slate"), css=custom_css, title=f"{APP_NAME} v{APP_VERSION}") as app:
        ...
        # Header
        with gr.Row():
            gr.Markdown(f"# {APP_NAME} v{APP_VERSION} 🚀\n*Fully Local • Advanced RAG • Autonomous Tools • Free Colab Setup*")
            status_txt = gr.Markdown("🟢 System Ready", scale=0)
            
        with gr.Tabs():
            # ── Main Chat Tab ──
            with gr.TabItem("💬 Console", id="chat"):
                with gr.Row():
                    with gr.Column(scale=3):
                        chatbot = gr.Chatbot(height=650, elem_id="chat-box", bubble_full_width=False, show_copy_button=True, avatar_images=(None, "🤖"))
                        
                        with gr.Group():
                            with gr.Row():
                                msg = gr.Textbox(show_label=False, placeholder="Ask anything, use tools, or upload files...", scale=8, container=False, lines=2)
                                send_btn = gr.Button("Send", variant="primary", scale=1)
                            
                            with gr.Row():
                                upload_btn = gr.UploadButton("📁 Upload to RAG", file_count="single", size="sm")
                                img_upload = gr.UploadButton("🖼️ Image (VL)", file_types=["image"], size="sm")
                                clear = gr.Button("🗑️ Clear", size="sm")
                                export_btn = gr.Button("💾 Export Chat", size="sm")
                                
                    with gr.Column(scale=1):
                        with gr.Accordion("⚙️ Agent Settings", open=True):
                            sys_p = gr.Textbox(label="System Prompt", value=SYSTEM_PROMPT, lines=3, max_lines=5)
                            sel_voice = gr.Dropdown(choices=["default", "british", "expressive"], value="default", label="AI Voice")
                            sel_adapter = gr.Dropdown(choices=["base", "coder", "scientist", "writer", "analyst"], value="base", label="Domain Expert (MoE)")
                        
                        with gr.Accordion("🛠️ Quick Tools", open=False):
                            gr.Markdown("_One-click access to core functionality_")
                            with gr.Row():
                                btn_search = gr.Button("Web Search", size="sm", elem_classes="tool-btn")
                                btn_browser = gr.Button("Auto Browser", size="sm", elem_classes="tool-btn")
                            with gr.Row():
                                btn_draw = gr.Button("Generate Image", size="sm", elem_classes="tool-btn")
                                btn_learn = gr.Button("Deep Research", size="sm", elem_classes="tool-btn")
                                
                        with gr.Accordion("📊 Live Status", open=False):
                            sys_status = gr.Markdown(system_status())
            ...
            with gr.TabItem("🎨 Studio"):
                ...
                with gr.Row():
                    with gr.Column(scale=1):
                        ...
                        img_neg = gr.Textbox(label="Negative Prompt", value="blurry, low quality, distorted")
                        img_steps = gr.Slider(1, 10, value=4, step=1, label="Steps (Turbo needs few)")
            ...
            with gr.TabItem("🧩 Plugins"):
                gr.Markdown("## Registered Tools & Custom Plugins")
                ...
                with gr.Row():
                    ...
                    pl_out = gr.Textbox(label="Status")
                    
            # ── Background Learner Tab ──
            with gr.TabItem("🧠 Continual Learning"):
                gr.Markdown("## Autonomous Background Learning (arXiv + RSS)")
                with gr.Row():
                    learn_status = gr.Textbox(label="Status Dashboard", value=learning_status(), lines=8, interactive=False)
                with gr.Row():
                    learn_topic = gr.Textbox(label="Add Topic (e.g., 'quantum computing')")
                    learn_add = gr.Button("Start Tracking")
                with gr.Row():
                    learn_start = gr.Button("Start Learner", variant="primary")
                    learn_stop = gr.Button("Stop Learner")
                    learn_refresh = gr.Button("Refresh Status")

            # ── Training & Alignment Tab ──
            with gr.TabItem("⚙️ Alignment"):
                gr.Markdown("## Local Alignment & Fine-tuning")
                with gr.Row():
                    with gr.Column():
                        gr.Markdown("### 1. RAG Audit")
                        audit_btn = gr.Button("Run QA Audit")
                        audit_out = gr.JSON(label="Audit Results")
                    with gr.Column():
                        gr.Markdown("### 2. Stream Training")
                        st_ds = gr.Dropdown(choices=list(DATASET_CATALOGUE.keys()), multiselect=True, label="Datasets")
                        st_btn = gr.Button("Start Training (Unsloth + Auto-Rollback)", variant="primary")
                        st_out = gr.Textbox(label="Training Log", lines=5)
                    with gr.Column():
                        gr.Markdown("### 3. DPO (Preferences)")
                        dpo_btn = gr.Button("Run DPO on User Corrections")
                        dpo_out = gr.Textbox(label="DPO Status")

        # --- Event Handlers ---
        
        # Chat
        def _chat(user_msg, hist, sys_p_val, img_p):
            if not user_msg.strip() and not img_p:
                yield hist, hist
                return
            
            # Apply system prompt changes
            global SYSTEM_PROMPT
            if sys_p_val and sys_p_val != SYSTEM_PROMPT:
                SYSTEM_PROMPT = sys_p_val
                
            p = user_msg
            if img_p:
                p = f"[Attached Image: {Path(img_p).name}] {p}"
            
            hist.append([p, ""])
            yield hist, hist

            final_ans = ""
            for token in agent_stream(user_msg, [{"role": "user" if i%2==0 else "assistant", "content": m} for i, pair in enumerate(hist[:-1]) for m in pair], img_path=img_p):
                final_ans = token
                hist[-1][1] = final_ans
                yield hist, hist

        msg.submit(_chat, [msg, history_state, sys_p, img_upload], [chatbot, history_state]).then(lambda: None, None, img_upload)
        send_btn.click(_chat, [msg, history_state, sys_p, img_upload], [chatbot, history_state]).then(lambda: None, None, img_upload)
        
        # Clear & Export
        clear.click(lambda: ([], []), None, [chatbot, history_state], queue=False)
        def _export(hist):
            if not hist: return "Nothing to export."
            md = f"# Exported Chat — {datetime.now().isoformat()}\n\n"
            for u, a in hist:
                md += f"**User:**\n{u}\n\n**{APP_NAME}:**\n{a}\n\n---\n"
            fp = f"{DIRS['ws_downloads']}/chat_export_{int(time.time())}.md"
            Path(fp).write_text(md)
            return f"✅ Exported to {fp}"
        export_btn.click(_export, [history_state], [status_txt])
        
        # Uploads
        def _handle_upload(f):
            if not f: return "No file"
            r = process_upload(f.name)
            return r
        upload_btn.upload(_handle_upload, upload_btn, status_txt)
        
        # Quick Tools
        btn_search.click(lambda: "Use the Web Search tool by typing: Search DuckDuckGo for [query]", None, msg)
        btn_browser.click(lambda: "Use the Auto Browser by typing: Navigate to [url] and [goal]", None, msg)
        btn_draw.click(lambda: "Generate an image of a cybernetic cat", None, msg)
        btn_learn.click(lambda: "Do deep research on recent advancements in solid state batteries", None, msg)
        
        # Dashboard Updates
        refresh_status.click(lambda: system_status(), None, sys_status)
        sel_adapter.change(load_moe_adapter, sel_adapter, status_txt)
        
        # Image Studio
        def _gen_img(p, np, s):
            r = generate_image(p, np, s)
            if r.startswith("__IMAGE__"):
                return r.replace("__IMAGE__", "")
            return None
        ...
        def _reload_pl():
            n = _load_plugins()
            return f"Reloaded. Total tools: {len(TOOL_REGISTRY)} ({n} from plugins)", [[k, v["desc"], str(v.get("params", {}))] for k, v in TOOL_REGISTRY.items()]
        pl_reload.click(_reload_pl, None, [pl_out, tool_df])
        
        # Continuous Learning
        learn_add.click(add_learning_topic, learn_topic, learn_status)
        learn_start.click(start_learning, None, learn_status)
        learn_stop.click(stop_learning, None, learn_status)
        learn_refresh.click(learning_status, None, learn_status)
        
        # Alignment
        audit_btn.click(lambda: rag_audit(), None, audit_out)
        st_btn.click(stream_train, st_ds, st_out)
        dpo_btn.click(dpo_train, None, dpo_out)
...
def _auto_save_worker():
    """Background thread: persist conversations and memory every 5 minutes."""
    while True:
        try:
            time.sleep(300)
            if _saved_convs:
                CONV_PATH.write_text(json.dumps(_saved_convs, indent=2))
            if _pmem:
                PMEM_PATH.write_text(json.dumps(_pmem, indent=2))
            if _dpo_pairs:
                DPO_PATH.write_text(json.dumps(_dpo_pairs, indent=2))
